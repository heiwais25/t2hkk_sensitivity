...
int main(int argc, char *argv[])
{
 ...
 //Iterate through the true values of deltacp
    ...
    {
        if(fSin23>=0 && itsin23!=fSin23) 
        {
            continue; //If a value was specified at the command line, only continue if at the value
        }
        
        double tsin23 = 0.4 + (double)itsin23*(0.2/100.); // Need to apply the range of sin theta 23
        sin23 = tsin23;
        ...
        for(int ithier = 0; ithier<2; ithier++)
        {
            ...
            if(fHierarchy>=0 && ithier!=fHierarchy) 
            {
                continue;
            }
            hierarchy = ithier;
