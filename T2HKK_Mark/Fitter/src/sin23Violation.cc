...
int main(int argc, char *argv[])
{
 ...
 TTree results("results","Sin23 violation significance");
 float min_known, min_unknown, sin23;
 int hierarchy;
 results.Branch("MinKnown",&min_known,"MinKnown/F");
 results.Branch("MinUnknown",&min_unknown,"MinUnknown/F");
 results.Branch("Sin23",&sin23,"Sin23/F");
 results.Branch("Hierarchy",&hierarchy,"Hierarchy/I");

 //Iterate through the true values of sin23
 for(int itsin23 = 0; itsin23<101; itsin23++){
   ...
   double tsin23 = 0.4 + (double)itsin23*(0.2/100.);
   sin23 = tsin23;
   ...
   for(int ithier = 0; ithier<2; ithier++){
     ...
     for(int ihier = 0; ihier<2; ihier++){
       ...
       //for(int ioct = 0; ioct<2; ioct++){
         ...
         for(int isin23 = 0; isin23<2; isin23++){
           ...
           //minuit->SetParameter(piter,Form("osc%d",oscParams->GetNOsc()-1),3.14159*(float)isin23,oscParams->GetParamError(oscParams->GetNOsc()-1)/100.0,
