...
int main(int argc, char *argv[])
{
 ...
 for(int itdcp = 0; itdcp<101; itdcp++){
   ...
   for(int ithier = 0; ithier<2; ithier++){
     ...
     for(int idcp = 0; idcp<19; idcp++){
       double fit_deltacp = tdcp+offsets[idcp]*TMath::Pi()/180.;
       double minimum = 9e9;
       ...
       std::cout << "Minimum at " << fit_deltacp << " is " << minimum << std::endl;
       gr->SetPoint(idcp,fit_deltacp,minimum);
     } //dcp loop  
