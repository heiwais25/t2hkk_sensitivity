...
int main(int argc, char *argv[])
{
 ...
 for(int i=0; i<6; i++)
 {
	 if(fSet1!=-1)
	 {
   		if( strcmp(pnames[i],fName1)==0)
   		{
	 		if(i == 0)
	 		{
		 		tset1 = fParam1 - 0.0001 + (float)fSet1 / 100. * 0.0002; // 100bin
		 		//tset1 = fParam1 - 0.00005 + (float)fSet1 / 40. * 0.0001;
	 		}
	 		else if(i == 1)
			 {
				 tset1 = fParam1 - 0.1 + (float)fSet1 / 100. * 0.2; // 100bin (-+ 0.2)
				 //tset1 = fParam1 - 0.1 + (float)fSet1 / 300. * 0.3; // 100bin (-+ 0.2)
				 //tset1 = fParam1 - 0.05 + (float)fSet1 / 40. * 0.1;
	 		} 
     		fixedParam[i] = true;
    		npoints[i] = 1;
    		startPoints[i][0] = tset1;
			std::cout << "fixed value is : " << fName1 << " = " << tset1 << std::endl; 
		}
	 }
	if(fSet2!=-1)
	{
   		if( strcmp(pnames[i],fName2)==0)
  		 {
			 if(i == 0)
			 {
				 tset2 = fParam2 - 0.0001 + (float)fSet2 / 100. * 0.0002; // 100bin
				 //tset2 = fParam2 - 0.00005 + (float)fSet2 / 40. * 0.0001;
			 }
			 else if(i == 1)
			 {
				 tset2 = fParam2 - 0.1 + (float)fSet2 / 100. * 0.2; // 100bin (-+ 0.2)
				 //tset2 = fParam2 - 0.05 + (float)fSet2 / 40. * 0.1;
			 }
		     fixedParam[i] = true;
    		 npoints[i] = 1;
		     startPoints[i][0] = tset2;
			 std::cout << "fixed value is : " << fName2 << " = " << tset2 << std::endl; 
  		 }
 	}
 }
 /* Setting hierarchy */
 ...
 /* Calculate fake experimental data */
 //Set the oscillation parameters and settings to calculate the prediction
 prob->SetOscillationParameters(startPoints[0][0],startPoints[1][0],oscParams->GetParamValue(2),
...
/* End of Calculate fake experimental data */
 ...
 if(tset2!=0)results.Branch("MinimumP2Value","TGraph",&tr,32000,0);

 results.Branch("P1Value",&p1value,"P1Value/F");
 if(tset2!=0)results.Branch("P2Value",&p2value,"P2Value/F");
 results.Branch("P1Name",&pname1);
 if(tset2!=0)results.Branch("P2Name",&pname2);
 ...
 for(ia[5]=0; ia[5]<npoints[5]; ia[5]++){ // dcp
   ...
   for(int i=0; i<systParams->GetNSysts(); i++)
   {
	 ...
	 if(density_kd == 2.6 && systParams->GetParamType(i)==MATTERDENSITY)
	 {
   ...
   for(int i=0; i<oscParams->GetNOsc(); i++)
   {
     ...
     if(fixedParam[i]) minuit->FixParameter(piter); // Fixing oscillation parameters we choose
     piter++;
   }  
   //Set the FCN
   minuit->SetFCN(myFcn);
 ...
 if(tset2!=0) value = fSet1 * 100 + fSet2;
 else value = fSet1;
 gr->SetPoint(value, p1value, minimum);
 if(tset2!=0)tr->SetPoint(value, p2value, minimum);
 gr->GetPoint(value, tempP1value, tempMinimum);
 if(tset2!=0)tr->GetPoint(value, tempP2value, tempMinimum);
 if(tset2!=0)
 {
	 std::cout << "value : " << value << " tSet1 : " << tset1 << " tSet2 : " << tset2 << " minimum : " << minimum << std::endl;
	 std::cout << "Temp value : " << value << " tSet1 : " << tempP1value << " tSet2 : " << tempP2value << " minimum : " << tempMinimum << std::endl;
 }
 else
 {
	 std::cout << "value : " << value << " tSet1 : " << tset1 <<  " minimum : " << minimum << std::endl;
	 std::cout << "Temp value : " << value << " tSet1 : " << tempP1value << " minimum : " << tempMinimum << std::endl;
 }
 //Fill the ttree
 results.Fill();
...
void myFcn(Int_t & /*nPar*/, Double_t * /*grad*/ , Double_t &fval, Double_t *p, Int_t /*iflag */  ){
  ...
  for(int i=0; i<systParams->GetNSysts(); i++)
  {
    ...
    if(systParams->GetParamType(i)==MATTERDENSITY)
	{
      ...
      if(systParams->DoDetector(2,i) && density_kd != 2.6) matterDensity[1] = p[i];
    }
  }
